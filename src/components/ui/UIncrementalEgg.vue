...
<template>
  <div class="ui-input flex" v-bind:class="{ focused: isFocused, flexible, invalid: egg.invalid, withLabel: label != undefined }">
    <span class="const prepend" @click="dec"><UIcon name="minus" /></span>
    <span class="empty" @click="$refs.input.focus()"></span>
    ...
    <div class="input-wrapper" @click="$refs.input.focus()">
      <span class="expander" v-text="internalValue" v-bind:class="{ wider: $attrs.type == 'number' }"></span>
...
.ui-input {
  ...
  &.focused {
    ...
    .prepend, .postpend { border-color: @col-text-dark; }
  }
  &.invalid {
    box-shadow: 0 0 0 1.5px @col-err;

    .prepend, .postpend { border-color: @col-err; }
  }
  .const {
    white-space: nowrap;
    flex: 0 0 0%;
...
export default defineComponent({
  ...
  methods: {
    inc() {
      this.egg.value = this.egg.rawValue + this.step;
    },
    dec() {
      this.egg.value = this.egg.rawValue - this.step;
    }
